...
namespace visuals
{
static float ColorToArrayBuf[4];

float* ColorToArray(ImColor color)
{
    ColorToArrayBuf[0] = color.Value.x;
    ColorToArrayBuf[1] = color.Value.y;
    ColorToArrayBuf[2] = color.Value.z;
    ColorToArrayBuf[3] = color.Value.w;
    return ColorToArrayBuf;
}
	void DrawFilledRect( int x, int y, int w, int h, float color[4] )
	{
		ImGui::GetBackgroundDrawList( )->AddRectFilled( ImVec2( x, y ), ImVec2( x + w, y + h ), IM_COL32( (int)(color[0]*255.f), (int)(color[1]*255.f), (int)(color[2]*255.f), (int)(color[3]*255.f) ) );
	}

	void DrawCornerBox( int x, int y, int w, int h, int borderPx, float color[4] )
	{
	...
	void DrawLabel( std::string text, glm::vec2 pos, float color[4], bool outline )
	{
		pos.x -= ( ImGui::CalcTextSize( text.c_str( ) ).x / 2.f );

		ImU32 outline_col = IM_COL32( 0, 0, 0, (int)( color[3] * 255.f ) );
		ImU32 text_col = IM_COL32( (int)( color[0] * 255.f ), (int)( color[1] * 255.f ), (int)( color[2] * 255.f ), (int)( color[3] * 255.f ) );

		if ( outline )
		{
			ImGui::GetBackgroundDrawList( )->AddText( ImVec2( pos.x, pos.y - 1 ), outline_col, text.c_str( ) );
			ImGui::GetBackgroundDrawList( )->AddText( ImVec2( pos.x, pos.y + 1 ), outline_col, text.c_str( ) );
			ImGui::GetBackgroundDrawList( )->AddText( ImVec2( pos.x - 1, pos.y ), outline_col, text.c_str( ) );
			ImGui::GetBackgroundDrawList( )->AddText( ImVec2( pos.x + 1, pos.y ), outline_col, text.c_str( ) );
			ImGui::GetBackgroundDrawList( )->AddText( ImVec2( pos.x + 1, pos.y + 1 ), outline_col, text.c_str( ) );
			ImGui::GetBackgroundDrawList( )->AddText( ImVec2( pos.x - 1, pos.y + 1 ), outline_col, text.c_str( ) );
			ImGui::GetBackgroundDrawList( )->AddText( ImVec2( pos.x + 1, pos.y - 1 ), outline_col, text.c_str( ) );
			ImGui::GetBackgroundDrawList( )->AddText( ImVec2( pos.x - 1, pos.y - 1 ), outline_col, text.c_str( ) );
		}

		ImGui::GetBackgroundDrawList( )->AddText( ImVec2( pos.x, pos.y ), text_col, text.c_str( ) );
	}
}
...
#endif
        ImGui::GetBackgroundDrawList( )->AddLine( ImVec2( a.x, a.y ), ImVec2( b.x, b.y ), (ImU32)color, 1.f );
    }
