...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImU32 col = GetColorU32(held ? ImVec4(0.20f, 0.45f, 0.80f, 1.0f) : hovered ? ImVec4(0.25f, 0.25f, 0.25f, 1.0f) : ImVec4(0.15f, 0.15f, 0.15f, 1.0f));
    ImU32 txt = GetColorU32(held ? ImVec4(0.9f, 0.9f, 1.0f, 1.0f) : ImVec4(1.0f, 1.0f, 1.0f, 1.0f));

    window->DrawList->AddRectFilled(bb.Min, bb.Max, col, style.FrameRounding);
    window->DrawList->AddText(center_text(bb.Min, bb.Max, label), txt, label);
...
bool ImGui::Checkbox(const char* label, bool* v, const char* hint)
{
    ...
    const ImRect check_bb(total_bb.Min, total_bb.Min + ImVec2(total_bb.GetSize().y, total_bb.GetSize().y));
    ItemSize(total_bb, style.FramePadding.y);
    ItemAdd(total_bb, id);
    ...
    ImU32 text_col = (ImU32)(*v ? text_color[0] : text_color[1]);
    ImU32 rect_col = (ImU32)(*v ? active_color : default_color);

    const char* label_end = FindRenderedTextEnd(label);

    window->DrawList->AddText(ImVec2(check_bb.Max.x + 10, center_text(total_bb.Min, total_bb.Max, label).y), text_col, label, label_end);

    float gt = (float)ImGui::GetTime();

    float last_toggle = window->StateStorage.GetFloat(id, -10.f);
    if (pressed) {
        last_toggle = gt;
        window->StateStorage.SetFloat(id, gt);
    }
    float toggle_age = gt - last_toggle;
    float toggle_anim = (toggle_age < 0.5f) ? (1.f - toggle_age / 0.5f) : 0.f;
    toggle_anim = toggle_anim * toggle_anim;

    ImU32 shadow_col = IM_COL32(0, 0, 0, 160);
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(3, 3), check_bb.Max + ImVec2(3, 3), shadow_col, 3.f);

    if (hovered) {
        float pulse = 0.3f + sinf(gt * 8.f) * 0.15f;
        for (int i = 0; i < 6; i++) {
            float ga = pulse * (1.f - (float)i / 6.f);
            ImU32 hg = IM_COL32(255, 0, 255, (int)(ga * 255.f));
            window->DrawList->AddRect(
                check_bb.Min - ImVec2((float)i + 1, (float)i + 1),
                check_bb.Max + ImVec2((float)i + 1, (float)i + 1),
                hg, 3.f, 0, 2.f);
        }
        ImU32 hf = IM_COL32(255, 0, 255, 50);
        window->DrawList->AddRectFilled(check_bb.Min - ImVec2(1, 1), check_bb.Max + ImVec2(1, 1), hf, 3.f);
    }

    ImU32 box_bg = *v ? (ImU32)active_color : (ImU32)default_color;
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, box_bg, 2.f);

    if (toggle_anim > 0.f) {
        float expand = toggle_anim * 12.f;
        for (int i = 0; i < 5; i++) {
            float ea = toggle_anim * (1.f - (float)i / 5.f) * 0.7f;
            ImU32 ec = IM_COL32(255, 0, 255, (int)(ea * 255.f));
            window->DrawList->AddRect(
                check_bb.Min - ImVec2(expand + (float)i, expand + (float)i),
                check_bb.Max + ImVec2(expand + (float)i, expand + (float)i),
                ec, 3.f + (float)i, 0, 2.f);
        }
        float flash = toggle_anim * 0.5f;
        ImU32 fc = IM_COL32(255, 100, 255, (int)(flash * 255.f));
        window->DrawList->AddRectFilled(
            check_bb.Min - ImVec2(expand * 0.5f, expand * 0.5f),
            check_bb.Max + ImVec2(expand * 0.5f, expand * 0.5f), fc, 3.f);
    }

    if (*v) {
        float pulse = 0.6f + sinf(gt * 5.f) * 0.35f;
        for (int i = 0; i < 5; i++) {
            float ga = pulse * (1.f - (float)i / 5.f) * 0.6f;
            ImU32 gc = IM_COL32(255, 0, 255, (int)(ga * 255.f));
            window->DrawList->AddRect(
                check_bb.Min - ImVec2((float)i + 1, (float)i + 1),
                check_bb.Max + ImVec2((float)i + 1, (float)i + 1),
                gc, 3.f, 0, 2.f);
        ...
        float half = check_bb.GetSize().y / 2 - 3;
        window->DrawList->AddRectFilled(check_bb.GetCenter() - ImVec2(half, half), check_bb.GetCenter() + ImVec2(half, half), rect_col, 1.f);

        for (int i = 0; i < 4; i++) {
            float ga = pulse * (1.f - (float)i / 4.f) * 0.45f;
            ImU32 gc = IM_COL32(255, 0, 255, (int)(ga * 255.f));
            window->DrawList->AddRect(
                check_bb.GetCenter() - ImVec2(half + (float)i + 1, half + (float)i + 1),
                check_bb.GetCenter() + ImVec2(half + (float)i + 1, half + (float)i + 1),
                gc, 1.f, 0, 1.5f);
        }

        ImU32 fill = IM_COL32(255, 0, 255, (int)(0.08f * 255.f));
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, fill, 2.f);
    }

    if (hint && CalcTextSize(hint).x > 2.f) {
        ImRect hint_rect(ImVec2(check_bb.Max.x + 15, total_bb.Min.y) + ImVec2(CalcTextSize(label).x + 3.f, 0.f), ImVec2(check_bb.Max.x + 15, total_bb.Min.y) + ImVec2(CalcTextSize(label).x + 3.f, 0.f) + CalcTextSize("(?)"));
        window->DrawList->AddText(center_text(hint_rect.Min, hint_rect.Max, "(?)"), active_color, "(?)");
        if (hint_rect.Contains(ImGui::GetMousePos()))
            ImGui::GetForegroundDrawList()->AddText(center_text(ImGui::GetMousePos() + ImVec2(15, 5), ImGui::GetMousePos() + ImVec2(30, 15) + CalcTextSize(hint), hint), active_color, hint);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float slider_height = 6.f;
    const float spacing_between = 4.f;
    ImVec2 total_size = ImVec2(w, label_size.y + spacing_between + slider_height);
    ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + total_size);
    const ImRect slider_bb(total_bb.Min + ImVec2(0.f, label_size.y + spacing_between), total_bb.Max);
    ...
    window->DrawList->AddRectFilled(slider_bb.Min, slider_bb.Max, (ImU32)default_color, slider_height * 0.5f);
    ...
    float grab_center_x = grab_bb.GetCenter().x;
    float fill_w = grab_center_x - slider_bb.Min.x;
    if (fill_w > 0.5f) {
        window->DrawList->PushClipRect(slider_bb.Min, ImVec2(grab_center_x, slider_bb.Max.y), true);
        window->DrawList->AddRectFilled(slider_bb.Min, slider_bb.Max, (ImU32)active_color, slider_height * 0.5f);
        window->DrawList->PopClipRect();
    }

    if (grab_bb.Max.x > grab_bb.Min.x) {
        ImVec2 gc = grab_bb.GetCenter();
        window->DrawList->AddCircleFilled(gc, 6.f, (ImU32)text_color[0], 360);
        window->DrawList->AddCircleFilled(gc, 3.f, (ImU32)ImColor(30, 30, 30, 200), 360);
    }
    ...
    const char* label_end = FindRenderedTextEnd(label);
    window->DrawList->AddText(total_bb.Min, (ImU32)text_color[0], label, label_end);
    ImVec2 value_size = CalcTextSize(value_buf);
    window->DrawList->AddText(ImVec2(total_bb.Max.x - value_size.x, total_bb.Min.y), (ImU32)text_color[0], value_buf);

    return value_changed;
}

void ImGui::SectionHeader(const char* label)
{
    ImGuiWindow* window = GetCurrentWindow();
    const ImGuiID id = window->GetID(label);
    ...
    const ImRect total_bb(pos, pos + ImVec2(w, text_size.y + 6));

    ItemSize(total_bb);
    const char* label_end = FindRenderedTextEnd(label);
    window->DrawList->AddText(pos, (ImU32)active_color, label, label_end);
    window->DrawList->AddLine(ImVec2(pos.x, pos.y + text_size.y + 4), ImVec2(pos.x + w, pos.y + text_size.y + 4), (ImU32)active_color, 1.0f);

    window->DC.CursorPos.y += text_size.y + 10;
}

void ImGui::SectionHeaderIcon(const char* label, int icon_type)
{
    ImGuiWindow* window = GetCurrentWindow();
    const ImGuiID id = window->GetID(label);
    ...
    const float w = ImGui::GetContentRegionAvail().x;
    const ImVec2 text_size = CalcTextSize(label);
    const float row_h = text_size.y + 12;
    const ImRect total_bb(pos, pos + ImVec2(w, row_h));

    ItemSize(total_bb, -1.0f);
    ItemAdd(total_bb, 0);

    const char* label_end = FindRenderedTextEnd(label);
    float box_size = 22.f;
    float box_x = pos.x;
    float box_y = pos.y + (row_h - box_size) * 0.5f;

    ImU32 purple = (ImU32)active_color;
    ImU32 purple_dim = GetColorU32(ImVec4(1.f, 0.f, 1.f, 0.15f));

    window->DrawList->AddRectFilled(
        ImVec2(box_x, box_y),
        ImVec2(box_x + box_size, box_y + box_size),
        IM_COL32(180, 0, 255, 100), 4.f);
    ...
    if (icon_type == 0)
    {
        window->DrawList->AddCircle(icon_center, 5.f, purple, 24, 1.2f);
        window->DrawList->AddCircleFilled(icon_center, 1.5f, purple, 12);
        window->DrawList->AddLine(ImVec2(icon_x, icon_center.y), ImVec2(icon_x + icon_size, icon_center.y), purple, 1.f);
        window->DrawList->AddLine(ImVec2(icon_center.x, icon_y), ImVec2(icon_center.x, icon_y + icon_size), purple, 1.f);
    }
    else if (icon_type == 1)
    {
        window->DrawList->AddRectFilled(ImVec2(icon_x + 1.f, icon_y + 2.f), ImVec2(icon_x + icon_size - 1.f, icon_y + icon_size - 1.f), purple, 1.f);
        window->DrawList->AddLine(ImVec2(icon_x + 3.f, icon_y + icon_size - 3.f), ImVec2(icon_x + icon_size - 3.f, icon_y + icon_size - 3.f), purple_dim, 1.f);
        window->DrawList->AddRect(ImVec2(icon_x + 1.f, icon_y + 2.f), ImVec2(icon_x + icon_size - 1.f, icon_y + icon_size - 1.f), purple, 1.f, 0, 1.f);
        window->DrawList->AddRectFilled(ImVec2(icon_center.x - 2.f, icon_y + icon_size - 2.f), ImVec2(icon_center.x + 2.f, icon_y + icon_size), purple, 0.f);
    }
    else if (icon_type == 2)
    {
        ImVec2 p1(icon_x + 2.f, icon_y + icon_size * 0.5f);
        ImVec2 p2(icon_center.x, icon_y + 1.f);
        ImVec2 p3(icon_x + icon_size - 2.f, icon_y + icon_size * 0.5f);
        ImVec2 p4(icon_center.x, icon_y + icon_size - 1.f);
        window->DrawList->AddQuadFilled(p1, p2, p3, p4, purple);
        window->DrawList->AddQuad(p1, p2, p3, p4, purple, 1.f);
        window->DrawList->AddCircleFilled(icon_center, 2.f, child_color[0], 12);
    }
    else if (icon_type == 3)
    {
        window->DrawList->AddCircle(icon_center, 5.f, purple, 24, 1.2f);
        for (int i = 0; i < 4; i++)
        {
            float a = (float)i / 4.f * 6.28318f;
            ImVec2 from(icon_center.x + 2.f * cosf(a), icon_center.y + 2.f * sinf(a));
            ImVec2 to(icon_center.x + 5.f * cosf(a), icon_center.y + 5.f * sinf(a));
            window->DrawList->AddLine(from, to, purple, 1.f);
        }
        window->DrawList->AddCircleFilled(icon_center, 1.f, purple, 8);
    }

    float text_x = box_x + box_size + 10.f;
    window->DrawList->AddText(ImVec2(text_x, pos.y + (row_h - text_size.y) * 0.5f), purple, label, label_end);

    float line_x1 = text_x + text_size.x + 10.f;
    for (int i = 0; i < 3; i++)
    {
        float a = 1.f - (float)i / 3.f;
        float ly = pos.y + row_h * 0.5f - 0.5f + (float)i;
        float alpha = (unsigned int)(a * 40.f);
        window->DrawList->AddRectFilled(
            ImVec2(line_x1, ly),
            ImVec2(pos.x + w, ly + 1.f),
            IM_COL32(180, 0, 255, alpha));
    }

    window->DC.CursorPos.y += row_h;
}
...
bool ImGui::Keybind(const char* label, int* key)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + frame_size + ImVec2(0, 30));
    const ImRect rect_bb(total_bb.Min + ImVec2(0, 18), total_bb.Max);
    ...
    const char* label_end = FindRenderedTextEnd(label);
    window->DrawList->AddRectFilled(rect_bb.Min, rect_bb.Max, default_color, 1.f);

    if (label[0] != '#' || label[1] != '#')
        window->DrawList->AddText(total_bb.Min, (ImU32)(kb_mode ? text_color[0] : text_color[1]), label, label_end);
    ...
    {
        if (*key < 250)
            display = GetKeyName(ImGuiKey(*key));
        else if (*key == 1001) display = "L.Mouse";
        else if (*key == 1002) display = "R.Mouse";
        else if (*key == 1003) display = "M.Mouse";
        else if (*key == 1004) display = "x1.Mouse";
        else if (*key == 1005) display = "x2.Mouse";
    }

    if (*key == 27) { kb_mode = false; *key = 0; }

    ImU32 kc = (ImU32)(kb_mode ? active_color : text_color[1]);
    window->DrawList->AddText(ImVec2(rect_bb.Min.x + 5.f, center_text(rect_bb.Min, rect_bb.Max, display).y), kc, display);
...
bool ImGui::SubTab(const char* label, int* v, int number)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(56 + CalcTextSize(label).x, 40));
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(total_bb, id, &hovered, &held);
    if (pressed) { *v = number; MarkItemEdited(id); }

    RenderNavCursor(total_bb, id);

    bool selected = (*v == number);
    ImU32 txt = selected ? (ImU32)text_color[0] : (ImU32)text_color[1];
    ImU32 line = selected ? (ImU32)active_color : IM_COL32(0, 0, 0, 0);
    ImU32 frame = IM_COL32(0, 0, 0, 0);

    float gt = (float)ImGui::GetTime();
    float hover_anim = hovered ? ImMin(gt * 5.f, 1.f) : 0.f;

    if (hovered && !selected) {
        float pulse = 0.2f + sinf(gt * 6.f) * 0.1f;
        ImU32 hbg = IM_COL32(255, 0, 255, (int)(pulse * 255.f));
        window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, hbg, style.FrameRounding);
        for (int i = 0; i < 5; i++) {
            ImU32 hg = IM_COL32(255, 0, 255, (int)(0.15f * (1.f - (float)i / 5.f) * 255.f));
            window->DrawList->AddRect(
                total_bb.Min - ImVec2((float)i + 1, (float)i + 1),
                total_bb.Max + ImVec2((float)i + 1, (float)i + 1),
                hg, style.FrameRounding + (float)i, 0, 1.5f);
        }
    }

    if (selected) {
        float sw = CalcTextSize(label).x;
        ImVec2 smin(total_bb.GetCenter().x - sw / 2, total_bb.Max.y - 3.f);
        ImVec2 smax(total_bb.GetCenter().x + sw / 2, total_bb.Max.y);
        for (int i = 0; i < 4; i++) {
            ImU32 fc = GetColorU32(ImVec4(1.f, 0.f, 1.f, 0.4f * (1.f - (float)i / 4.f)));
            window->DrawList->AddRectFilled(
                ImVec2(smin.x - (float)i, smin.y - (float)i),
                ImVec2(smax.x + (float)i, smax.y + (float)i), fc, 2.f);
        }
    }

    window->DrawList->AddRectFilled(
        ImVec2(total_bb.GetCenter().x - CalcTextSize(label).x / 2, total_bb.Max.y - 3.f),
        ImVec2(total_bb.GetCenter().x + CalcTextSize(label).x / 2, total_bb.Max.y),
        line, 360.f, ImDrawFlags_RoundCornersTop);

    window->DrawList->AddText(center_text(total_bb.Min, total_bb.Max, label), txt, label);

    return pressed;
}

static ImTextureID g_AimIconTexture = NULL;
static ImTextureID g_VisualsIconTexture = NULL;
static ImTextureID g_MiscIconTexture = NULL;
static ImTextureID g_SettingsIconTexture = NULL;
void ImGui::SetAimIconTexture(ImTextureID tex) { g_AimIconTexture = tex; }
void ImGui::SetVisualsIconTexture(ImTextureID tex) { g_VisualsIconTexture = tex; }
void ImGui::SetMiscIconTexture(ImTextureID tex) { g_MiscIconTexture = tex; }
void ImGui::SetSettingsIconTexture(ImTextureID tex) { g_SettingsIconTexture = tex; }

bool ImGui::Tab(const char* icon, int* v, int number)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(38, 38));
    ...
    bool selected = (*v == number);
    ImU32 col = GetColorU32(ImVec4(selected ? active_color : text_color[1]));

    float gt = (float)ImGui::GetTime();
    if (hovered && !selected) {
        float pulse = 0.25f + sinf(gt * 7.f) * 0.12f;
        ImVec2 center = total_bb.Min + ImVec2(19.f, 19.f);
        for (int i = 0; i < 5; i++) {
            float ga = pulse * (1.f - (float)i / 5.f);
            ImU32 hg = IM_COL32(255, 0, 255, (int)(ga * 255.f));
            window->DrawList->AddCircleFilled(center, 9.f + (float)i * 2.f, hg, 32);
        }
    }
    ...
    if (selected) {
        ImVec2 center = total_bb.Min + ImVec2(19.f, 19.f);
        for (int i = 0; i < 4; i++) {
            ImU32 fade = GetColorU32(ImVec4(1.f, 0.f, 1.f, 0.35f * (1.f - (float)i / 4.f)));
            window->DrawList->AddCircleFilled(center, 7.f + (float)i * 1.5f, fade, 32);
        }
    }

    ImVec2 c = total_bb.GetCenter();
    float r = 8.f;

    if (number == 0) {
        if (g_AimIconTexture) {
            float s = 13.f;
            window->DrawList->AddImage(g_AimIconTexture, ImVec2(c.x - s, c.y - s), ImVec2(c.x + s, c.y + s));
        } else {
            window->DrawList->AddCircle(c, 8.f, col, 64, 1.5f);
            window->DrawList->AddCircleFilled(c, 1.5f, col, 16);
            window->DrawList->AddLine(ImVec2(c.x - 12.f, c.y), ImVec2(c.x - 3.f, c.y), col, 1.5f);
            window->DrawList->AddLine(ImVec2(c.x + 3.f, c.y), ImVec2(c.x + 12.f, c.y), col, 1.5f);
            window->DrawList->AddLine(ImVec2(c.x, c.y - 12.f), ImVec2(c.x, c.y - 3.f), col, 1.5f);
            window->DrawList->AddLine(ImVec2(c.x, c.y + 3.f), ImVec2(c.x, c.y + 12.f), col, 1.5f);
        }
    }
    else if (number == 1) {
        if (g_VisualsIconTexture) {
            float s = 13.f;
            window->DrawList->AddImage(g_VisualsIconTexture, ImVec2(c.x - s, c.y - s), ImVec2(c.x + s, c.y + s));
        } else {
            window->DrawList->AddCircle(c, 8.f, col, 64, 1.5f);
            window->DrawList->AddCircleFilled(c, 2.f, col, 16);
            window->DrawList->AddCircle(c, 5.f, col, 48, 1.5f);
        }
    }
    else if (number == 2) {
        if (g_MiscIconTexture) {
            float s = 13.f;
            window->DrawList->AddImage(g_MiscIconTexture, ImVec2(c.x - s, c.y - s), ImVec2(c.x + s, c.y + s));
        } else {
            int teeth = 8;
            float inner_r = r - 1.5f;
            float outer_r = r + 1.5f;
            for (int i = 0; i < teeth; i++) {
                float a = (float)i / (float)teeth * 6.28318f;
                float hw = 3.14159f / (float)teeth * 0.5f;
                ImVec2 p1(c.x + inner_r * cosf(a - hw), c.y + inner_r * sinf(a - hw));
                ImVec2 p2(c.x + outer_r * cosf(a - hw), c.y + outer_r * sinf(a - hw));
                ImVec2 p3(c.x + outer_r * cosf(a + hw), c.y + outer_r * sinf(a + hw));
                ImVec2 p4(c.x + inner_r * cosf(a + hw), c.y + inner_r * sinf(a + hw));
                window->DrawList->AddQuadFilled(p1, p2, p3, p4, col);
            }
            window->DrawList->AddCircle(c, inner_r, col, 48, 1.5f);
            window->DrawList->AddCircle(c, 3.f, col, 32, 1.5f);
            window->DrawList->AddCircleFilled(c, 1.5f, col, 16);
        }
    }
    else if (number == 3) {
        if (g_SettingsIconTexture) {
            float s = 13.f;
            window->DrawList->AddImage(g_SettingsIconTexture, ImVec2(c.x - s, c.y - s), ImVec2(c.x + s, c.y + s));
        } else {
            window->DrawList->AddCircle(c, 7.f, col, 48, 1.5f);
            window->DrawList->AddCircleFilled(c, 3.f, col, 32);
            for (int i = 0; i < 4; i++) {
                float a = (float)i / 4.f * 6.28318f;
                ImVec2 p1(c.x + 5.f * cosf(a), c.y + 5.f * sinf(a));
                ImVec2 p2(c.x + 9.f * cosf(a), c.y + 9.f * sinf(a));
                window->DrawList->AddLine(p1, p2, col, 2.f);
            }
        }
    }
...
bool ImGui::ComboHQ(const char* label, int* current_item, const char* const items[], int items_count)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    float combo_h = 46.f;
    float item_h = 22.f;
    static ImGuiID open_combo_id = 0;
    static ImRect open_dropdown_rect;
    static int dropdown_active_frame = -1;
    bool is_open = (open_combo_id == id);

    if (g.FrameCount != dropdown_active_frame)
        dropdown_active_frame = -1;

    if (dropdown_active_frame >= 0 && open_dropdown_rect.Contains(g.IO.MousePos))
    {
        g.HoveredId = 99999;
        ItemSize(ImVec2(frame_size.x, combo_h), style.FramePadding.y);
        return false;
    }

    const ImRect combo_bb(pos, pos + ImVec2(frame_size.x, combo_h));
    const ImRect label_bb(pos, pos + ImVec2(frame_size.x, 16));
    const ImRect rect_bb(pos + ImVec2(0, 18), pos + ImVec2(frame_size.x, 46));

    ItemSize(combo_bb, style.FramePadding.y);
    if (!ItemAdd(combo_bb, id))
        return false;
    ...
    window->DrawList->AddText(label_bb.Min, text_color[1], label, label_end);

    ImU32 box_bg = (hovered || is_open) ? (ImU32)child_color[0] : (ImU32)default_color;
    window->DrawList->AddRectFilled(rect_bb.Min, rect_bb.Max, box_bg, 4.f);
    window->DrawList->AddRect(rect_bb.Min, rect_bb.Max, is_open ? (ImU32)active_color : (ImU32)stroke_color, 4.f, 0, 1.f);

    window->DrawList->AddText(
        ImVec2(rect_bb.Min.x + 8.f, center_text(rect_bb.Min, rect_bb.Max, preview).y),
        (ImU32)text_color[1], preview);

    char num_buf[4];
    snprintf(num_buf, sizeof(num_buf), "%d", *current_item + 1);
    window->DrawList->AddText(
        ImVec2(rect_bb.Max.x - CalcTextSize(num_buf).x - 8.f, center_text(rect_bb.Min, rect_bb.Max, num_buf).y),
        (ImU32)text_color[0], num_buf);

    if (is_open)
    {
        ImVec2 drop_min(pos.x, pos.y + combo_h);
        ImVec2 drop_max(pos.x + frame_size.x, pos.y + combo_h + items_count * item_h);
        open_dropdown_rect = ImRect(drop_min, drop_max);
        dropdown_active_frame = g.FrameCount;

        ImDrawList* dl = ImGui::GetForegroundDrawList();
        dl->AddRectFilled(drop_min, drop_max, IM_COL32(12, 10, 18, 255), 4.f);
        dl->AddRect(drop_min, drop_max, (ImU32)stroke_color, 4.f, 0, 1.f);

        bool clicked_any = false;

        for (int i = 0; i < items_count; i++)
        {
            float iy = drop_min.y + i * item_h;
            ImVec2 item_min(drop_min.x + 1, iy);
            ImVec2 item_max(drop_max.x - 1, iy + item_h);

            bool item_hovered = (g.IO.MousePos.x >= item_min.x && g.IO.MousePos.x <= item_max.x &&
                               g.IO.MousePos.y >= item_min.y && g.IO.MousePos.y <= item_max.y);

            if (item_hovered)
                dl->AddRectFilled(item_min, item_max, (ImU32)child_color[0], 2.f);

            dl->AddRect(item_min, item_max, (ImU32)stroke_color, 2.f, 0, 1.f);

            ImU32 item_col = (i == *current_item) ? (ImU32)active_color : (ImU32)text_color[1];
            dl->AddText(
                ImVec2(item_min.x + 8.f, center_text(item_min, item_max, items[i]).y),
                item_col, items[i]);

            if (item_hovered && g.IO.MouseClicked[0])
            {
                *current_item = i;
                open_combo_id = 0;
                is_open = false;
        ...
        if (open_dropdown_rect.Contains(g.IO.MousePos))
        {
            g.IO.MouseClicked[0] = false;
            g.IO.MouseReleased[0] = false;
        }
    }
...
bool ImGui::KeybindHQ(const char* label, int* key)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(frame_size.x, 46));
    const ImRect label_bb(pos, pos + ImVec2(frame_size.x, 16));
    const ImRect rect_bb(pos + ImVec2(0, 18), pos + ImVec2(frame_size.x, 46));
    ...
    bool is_listening = (ImGui::GetActiveID() == id);
    ...
    else if (*key < 251) display = GetKeyName(ImGuiKey(*key));

    const char* label_end = FindRenderedTextEnd(label);
    window->DrawList->AddText(label_bb.Min, text_color[1], label, label_end);

    ImU32 box_col = is_listening ? (ImU32)active_color : (hovered ? (ImU32)child_color[0] : (ImU32)default_color);
    window->DrawList->AddRectFilled(rect_bb.Min, rect_bb.Max, box_col, 4.f);
    window->DrawList->AddRect(rect_bb.Min, rect_bb.Max, is_listening ? (ImU32)stroke_color_active : (ImU32)stroke_color, 4.f, 0, 1.f);

    ImU32 txt = is_listening ? (ImU32)active_color : (*key != 0 ? (ImU32)text_color[0] : (ImU32)text_color[1]);
    window->DrawList->AddText(
        ImVec2(rect_bb.Min.x + 8.f, center_text(rect_bb.Min, rect_bb.Max, display).y),
        txt, display);
