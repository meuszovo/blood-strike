...
namespace ImGui
{
    ...
    // Widgets: Custom (Tab, SubTab, Keybind)
    IMGUI_API bool          Tab(const char* icon, int* v, int number);
    IMGUI_API bool          SubTab(const char* label, int* v, int number);
    IMGUI_API bool          Keybind(const char* label, int* key);
    IMGUI_API void          SectionHeader(const char* label);
    IMGUI_API void          SectionHeaderIcon(const char* label, int icon_type);
    IMGUI_API void          SetAimIconTexture(ImTextureID tex);
    IMGUI_API void          SetVisualsIconTexture(ImTextureID tex);
    IMGUI_API void          SetMiscIconTexture(ImTextureID tex);
    IMGUI_API void          SetSettingsIconTexture(ImTextureID tex);

    // Widgets: HQ Style (dark themed, label-on-top, indicator)
    IMGUI_API bool          ComboHQ(const char* label, int* current_item, const char* const items[], int items_count);
    IMGUI_API bool          KeybindHQ(const char* label, int* key);
